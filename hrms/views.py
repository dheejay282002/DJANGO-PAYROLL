...
def dashboard(request):
    ...
    gen_settings, created = General_settings.objects.get_or_create(
        id=1,
        defaults={
            'main_company': 'Default Company',  # replace with your actual default
            # add any other required non-null fields here
        }
    )
    ...
    context = {
        ...
        'head': 'Dashboard',
        # include any other context you need
    }
...
def search_company(request):
    ...
    return render(request, 'hrms/search_results.html', context)
...
#    ws.row(0).write(0, value, xlwt.Style.easyxf(style))
    ws.write_merge(0, 1, 0, 20, f"Luxor Manpower- {company.company_name}'s Employee List",
    ...
    columns = ['Lastname', 'Firstname', 'Middlename', 'Date Hired', 'Date End', 'Birthday', 'SSS',
               'PAGIBIG', 'PHILHEALTH', 'CONTACT NUMBER']
    ...
    rows = Employee.objects.filter(company=pk).values_list(
        'last_name', 'first_name', 'middle_name', 'date_hired', 'contract_expiration', 'date_of_birth', 'sss_no',
        'pagibig_no', 'philhealth_no', 'phone'
    )
...
def company_show(request):
    ...
    return render(request, 'hrms/all_companies.html', context)
...
def company_add(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(
                request, f'New company {company.company_name} was successfully created.')
            return redirect('company-show')
    else:
        form = CompanyAddForm()

    # Fix: get_or_create ensures there is always a General_settings row
    gen_settings, created = General_settings.objects.get_or_create(
        id=1,
        defaults={
            'main_company': 'Default Company',  # replace with your actual required fields
            # add any other fields here that cannot be null
        }
    )
    ...
    return render(request, 'hrms/company_add.html', context)

@login_required# delete company
