...
def employee_data_movement(request, pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            # This will print every field error to your console/log
    ...
    context = {
        'head': f"Employee Data Movement - {employee.first_name} {employee.last_name}",
        'form': form,
        'pk': pk,
        'employee': employee,
        'logs': logs
...
def employee_add(request):
    if request.method == 'POST':
        ...
        try:
            ...
            if not company_id:
                messages.warning(request, "Please select a valid company.")
                return redirect("employee-add")
            company = get_object_or_404(Company, pk=company_id)
        except ValueError:
            messages.warning(request, "Invalid company ID provided.")
            return redirect("employee-add")

        if empAddForm.is_valid():
            citizenship = empAddForm.cleaned_data['citizenship']
            citizenship_option = request.POST.get('citizenship_option', '')

            # Check for duplicate names
            names = Employee.objects.filter(
                first_name=empAddForm.cleaned_data['first_name'],
                middle_name=empAddForm.cleaned_data['middle_name'],
                last_name=empAddForm.cleaned_data['last_name']
            ).first()
            if names:
                messages.error(
                    request,
                    f"Employee {empAddForm.cleaned_data['first_name']} {empAddForm.cleaned_data['middle_name']} {empAddForm.cleaned_data['last_name']} already exists."
                )
                return redirect("employee-add")
            ...
            try:
                if citizenship == "Others":
                    ...
                    # Add new citizenship
                    citiz = Employee_citizenship(citizenship=citizenship_option)
                    citiz.save()
                ...
                action = f"Employee {request.POST.get('first_name', '')} {request.POST.get('middle_name', '')} {request.POST.get('last_name', '')} was successfully added to {company} company."
                Logs.objects.create(
                    action=action,
                ...
                messages.success(request, 'Employee has been successfully saved.')
            ...
            messages.warning(request, "Form is not valid. Please check the fields.")
            return redirect("employee-add")
