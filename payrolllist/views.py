...
def gov_benefits(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            Logs.objects.create(
                action=f"Gov benefits report generated. (company - {company}, start_date - {start_date}, end_date - {end_date}, payment_method - {payment_method})",
            ...
            style = xlwt.easyxf(
                'align: wrap on, vert center, horiz center; font: bold on; borders: left thin, right thin, top thin, bottom thin;'
            )
            style_basic = xlwt.easyxf(
                'align: wrap on, vert center, horiz center; borders: left thin, right thin, top thin, bottom thin;'
            )

            # Static headers
            ws.write_merge(0, 0, 0, 9, f"{company} {start_date} - {end_date}", style)
            ws.write_merge(1, 1, 0, 0, "Last name", style)
            ws.write_merge(1, 1, 1, 1, "First name", style)
            ws.write_merge(1, 1, 2, 2, "Middle Name", style)
            ws.write_merge(1, 1, 3, 3, "Rate", style)
            ws.write_merge(1, 1, 4, 4, "Birthday", style)
            ws.write_merge(1, 1, 5, 5, "SSS#", style)
            ws.write_merge(1, 1, 6, 6, "PHILHEALTH#", style)
            ws.write_merge(1, 1, 7, 7, "PAGIBIG#", style)
            ws.write_merge(1, 1, 8, 8, "MOBILE NO.#", style)

            # Write data rows for base payroll
            for row_num, payroll in enumerate(base_payroll, start=2):
                ws.write(row_num, 0, payroll.last_name, style_basic)
                ws.write(row_num, 1, payroll.first_name, style_basic)
                ws.write(row_num, 2, payroll.middle_name, style_basic)
                ws.write(row_num, 3, str(payroll.rate), style_basic)
                ws.write(row_num, 4, str(payroll.birthday), style_basic)
                ws.write(row_num, 5, payroll.sss_number, style_basic)
                ws.write(row_num, 6, payroll.philhealth_number, style_basic)
                ws.write(row_num, 7, payroll.pagibig_number, style_basic)
                ws.write(row_num, 8, payroll.mobile_number, style_basic)

            # Dynamic gov benefits logic
            row = 3
            col = 10
            listahan = []
            emp_counter = 0
            ...
            for idx, base in enumerate(base_payroll):
                ...
                ws.write_merge(1, 1, col, col+1, f"{base.start_date} - {base.end_date}", style)
                ws.write_merge(2, 2, col, col, "DAYS", style)
                ws.write_merge(2, 2, col+1, col+1, "SSS/PH/PAG", style)

                if str(company).lower() == "phil asia":
                    payroll_listhan = Payroll_for_phil_asia.objects.filter(base_payroll=base.pk).order_by('employee__last_name')
                else:
                ...
                for pay in payroll_listhan:
                    if pay.employee.pk not in listahan:
                        ...
                        ws.write_merge(row, row, 0, 0, emp_counter + 1, style)
                        ws.write_merge(row, row, 1, 1, pay.employee.last_name, style_basic)
                        ws.write_merge(row, row, 2, 2, pay.employee.first_name, style_basic)
                        ws.write_merge(row, row, 3, 3, pay.employee.middle_name, style_basic)
                        ws.write_merge(row, row, 4, 4, pay.employee.get_hiring_details().rate, style_basic)
                        ws.write_merge(row, row, 5, 5, pay.employee.date_of_birth, style_basic)
                        ws.write_merge(row, row, 6, 6, pay.employee.sss_no, style_basic)
                        ws.write_merge(row, row, 7, 7, pay.employee.philhealth_no, style_basic)
                        ws.write_merge(row, row, 8, 8, pay.employee.pagibig_no, style_basic)
                        ws.write_merge(row, row, 9, 9, pay.employee.phone, style_basic)
                        row += 1
                        emp_counter += 1
                col += 2

            # Fill dynamic DAYS/DEDUCTIONS and create Gov_benefits entries
            row = 3
            base_col = 9
                        ...
                        for tao in listahan:
                            for pays in payroll_listhan:
                                if tao == pays.employee_id:
                                    ...
                                    days_val = pays.days if str(company).lower() == "phil asia" else pays.regular_days
                                    ...
                                    ws.write_merge(row, row, base_col+1, base_col+1, days_val, style)
                                    ws.write_merge(row, row, base_col+2, base_col+2, deductions, style)
                            row += 1
                        base_col += 2
                        row = 3

            # Write total DAYS and DEDUCTIONS per employee
            row = 3
            for tao in listahan:
                total_days = 0
                total_deductions = 0
                for base in base_payroll:
                    tao_data = Gov_benefits.objects.filter(employee_id=tao, start_date=base.start_date, end_date=base.end_date).first()
                    if tao_data:
                        total_days += tao_data.days
                        total_deductions += tao_data.deductions
                ws.write_merge(row, row, base_col+1, base_col+1, total_days, style)
                ws.write_merge(row, row, base_col+2, base_col+2, total_deductions, style)
                row += 1

            # Write totals header
            ws.write_merge(1, 1, col, col, "Total", style)
            ws.write_merge(2, 2, col, col, "Days", style)
            ws.write_merge(1, 1, col+1, col+1, "Total", style)
            ws.write_merge(2, 2, col+1, col+1, "DEDUCTION", style)

            # Clear Gov_benefits table
            Gov_benefits.objects.all().delete()

            wb.save(response)
            return response

        return render(request, 'payroll/gov_benefits.html', {'form': form})
